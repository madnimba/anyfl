"""RGAR (Reference-Guided Attribution + Reconstruction) for K=2 legacy flatten VFL.

Aligns with ``server_rgar_defense.py`` / ``attack_core.train_once_rgar`` but runs on
:class:`vfl.models.legacy_flat_vfl.KPartyLegacyFlattenVFL` (MNIST / Fashion-MNIST
default stack). Party **A** = attacker client index (swapped view); party **B** =
the passive clean view.

Threat model (blueprint): server holds labels, a **small stratified reference**
subset of train indices is **not swapped** on the attacker side; reference
statistics and ``g(h_B, y)`` are fit on that clean slice; online suspicion +
optional reconstruction / downweight mitigate cluster-swap poisoning.
"""
...
class _ServerConcat(nn.Module):
    """Expose ``KPartyLegacyFlattenVFL.server`` as ``forward(ha, hb)``."""
...
def train_rgar_flatten_vfl(
    ...
    *,
    ...
    """Train with RGAR mitigation on poisoned attacker data (K=2 only).

    Returns ``(test_metrics, train_stats, rgar_meta)``.
    """
    if len(X_parts_clean) != 2 or len(X_parts_poison) != 2:
        raise ValueError("train_rgar_flatten_vfl currently supports k_clients=2 only")
    if attacker_idx not in (0, 1):
        raise ValueError("attacker_idx must be 0 or 1")
    if not isinstance(model, KPartyLegacyFlattenVFL):
        raise TypeError("model must be KPartyLegacyFlattenVFL for this defense path")
    ...
    # --- Reference warm-up (clean attacker, paired with passive on ref rows) ---
    model.train()
